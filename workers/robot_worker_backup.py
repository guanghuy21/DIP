...
class RobotThread(threading.Thread):
    ...
    def fetch_lidar_points(self, target_url, data_path):
        """
        Fetches processed point data from a specific LiDAR API endpoint.
        :param target_url: The full URL (e.g., http://127.0.0.1:5000/lidar/process_data)
        :param data_path: The filename/path to process
        """
        payload = {"path": data_path}
        
        try:
            # We send a POST request to the provided URL with the path in the body
            ...
            # Log the error and return None so the route can handle the failure
            print(f"RobotWorker Error: {e}")
            return None
